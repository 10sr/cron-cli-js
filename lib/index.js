...
var PARSE_CRONTAB_RE = new RegExp(/^\s*(\S+)\s+(\S+)\s+(\S+)\s+(\S+)\s+(\S+)\s+(\S+)\s+(.+)$/);
...
function parseCrontab(line){
    var matched = PARSE_CRONTAB_RE.exec(line);
    if (!matched) {
    ...
    rl.on("line", function(line){
        ...
        var cronJobOpts = {
            ...
            onTick: function(){
                ...
                leadingMarker = parsed.command +
                    " (pid=" + proc.pid.toString() + ")" + " | ";
                logger.info(leadingMarker + "Started.")
            },
            start: true
        };
        if (timeZone) {
        ...
        logger.info("Job registered: " + JSON.stringify(parsed));
    });
};
